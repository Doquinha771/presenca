...
try{
 ...
 for(const name of ['overview','entry','students','history','enrollments','classes','team','adjustments','announcements','archive','period','audit','privacy','importer']){await page.goto(base+'#/'+name);await page.waitForFunction(n=>document.querySelector('#nav a.active')?.dataset.page===n,name);await page.waitForFunction(()=>!document.getElementById('view').hasAttribute('aria-busy'));assert.equal(await page.locator('#view').getByText('Não foi possível carregar os dados',{exact:true}).count(),0);checks++;}
 await page.goto(base+'#/importer');
 await page.locator('#importForm').waitFor();
 await page.locator('#importKind').selectOption('classes');
 await page.locator('#importFile').setInputFiles({name:'turmas.csv',mimeType:'text/csv',buffer:Buffer.from('Série;Turma\n3º ano;A\n3º ano;B\n')});
 await page.locator('#importPreview').click();
 await page.locator('.import-summary').waitFor();
 await page.locator('#confirmImport').check();
 await page.locator('#importResults > button').click();
 await page.getByText('Importação finalizada:',{exact:false}).waitFor();
 assert.equal(await page.evaluate(()=>window.__calls.some(x=>x.name==='portal_import_batch')),true);
 checks++;
