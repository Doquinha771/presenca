...
-- Importação institucional 5.0: autorização de cargo, duplicidades e período.
set local role authenticated;
set local request.jwt.claim.sub='a4000000-0000-4000-8000-000000000002';
select pg_temp.denied($q$select public.portal_import_batch('classes','[{"grade":"3º ano teste","name":"Nova"}]'::jsonb)$q$,'reservada à Direção');
select pg_temp.check_true((public.portal_import_batch('enrollments','[{"line":1,"ra":"9999999999077","name":"Aluno Importado","birth":"2007-01-01","grade":"3º ano teste","class":"T"}]'::jsonb)->>'added')::integer=1,'secretaria importa matrícula sem acesso administrativo');
select pg_temp.check_true((public.portal_import_batch('enrollments','[{"line":1,"ra":"9999999999077","name":"Aluno Importado","birth":"2007-01-01","grade":"3º ano teste","class":"T"}]'::jsonb)->>'skipped')::integer=1,'reimportação de matrícula não cria cópia');
set local request.jwt.claim.sub='a4000000-0000-4000-8000-000000000001';
select pg_temp.check_true((public.portal_import_batch('classes','[{"line":1,"grade":"3º ano teste","name":"Nova"}]'::jsonb)->>'added')::integer=1,'direção importa nova turma');
select pg_temp.check_true((public.portal_import_batch('classes','[{"line":1,"grade":"3º ano teste","name":"Nova"}]'::jsonb)->>'skipped')::integer=1,'reimportação de turma é idempotente');
select pg_temp.check_true((public.portal_import_batch('lateness','[{"line":1,"ra":"9999999999001","date":"2026-09-01","time":"07:35","reason":"Histórico anterior ao período","justified":false,"request":"a4000000-0000-4000-8000-000000000085"}]'::jsonb)->>'added')::integer=1,'direção importa atraso histórico');
select pg_temp.check_true((public.portal_import_batch('lateness','[{"line":1,"ra":"9999999999001","date":"2026-09-01","time":"07:35","reason":"Histórico anterior ao período","justified":false,"request":"a4000000-0000-4000-8000-000000000085"}]'::jsonb)->>'skipped')::integer=1,'histórico idempotente na reimportação');
reset role;
select pg_temp.check_true((select unjustified_count=4 from public.profiles where id='a4000000-0000-4000-8000-000000000003'),'histórico anterior ao período não altera contagem corrente');
select pg_temp.check_true((select count(*)=1 from public.attendance_events where request_id='a4000000-0000-4000-8000-000000000085'),'histórico gravado uma única vez');
